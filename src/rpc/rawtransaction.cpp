...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the aprncoin address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
    ...
    for (const std::string& name_ : addrList) {
        ...
        if (name_ == "data") {
            ...
            CTxOut out(0, CScript() << OP_REPORT << data);
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) aprncoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
