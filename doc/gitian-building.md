...
*Setup instructions for a Gitian build of aprncoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the aprncoin
...
- [Building aprncoin Core](#building-aprncoin-core)
...
Clone the git repositories for aprncoin and Gitian.
...
in the aprncoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building aprncoin Core
----------------

To build aprncoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the aprncoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/aprncoin/.git/
...
For example:
```bash
URL=https://github.com/thrasher-/aprncoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit aprncoin=${COMMIT} --url aprncoin=${URL} ../aprncoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit aprncoin=${COMMIT} --url aprncoin=${URL} ../aprncoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit aprncoin=${COMMIT} --url aprncoin=${URL} ../aprncoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the aprncoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../aprncoin/contrib/gitian-descriptors/*|sort|uniq )
...
./bin/gbuild --url aprncoin=${BTCPATH},signature=${SIGPATH} ../aprncoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/aprncoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/aprncoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/aprncoin-osx-build.assert
```
