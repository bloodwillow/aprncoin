...
Run with the -testnet option to run with "play aprncoins" on the test network, if you
...
aprncoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 1947.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 1942 for connections and services them.

- BitcoinMiner : Generates aprncoins (if wallet is enabled).
...
However, in open source software such as aprncoin Core, where everyone uses
...
pay attention to for reviewers of aprncoin Core code.

General aprncoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: aprncoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of aprncoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against aprncoin Core so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:aprncoin-project/aprncoin.git
