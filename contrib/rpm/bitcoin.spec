...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 1942
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 1947
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19241
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19741
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 1942
	%{_sbindir}/semanage port -d -p tcp 1947
	%{_sbindir}/semanage port -d -p tcp 19241
	%{_sbindir}/semanage port -d -p tcp 19741
	for selinuxvariant in %{selinux_variants}; do
